...
question_add = {
	...
	composeEntity : function composeEntity() {
		...
		question_entity.questionTypeId = $(".question-type select").val();

		var points = "";
		var pointOpts = $("#point-to-select option");
		for (var i = 0; i < pointOpts.length; i++) {
			points = points+","+($(pointOpts[i]).attr("value"));
		}
		question_entity.points = points+",";

		if (1 == question_entity.questionTypeId) {
			question_entity.answer = $(".form-question-answer1 select").val();
		} else if (2 == question_entity.questionTypeId) {
		...
		} else if (3 == question_entity.questionTypeId) {
